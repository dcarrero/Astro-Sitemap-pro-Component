...
export function buildRobotsTxt(opts = {}) {
    const lines = [`User-agent: ${opts.userAgent ?? "*"}`];
    ...
    const sitemaps = opts.sitemaps == null ? [] : Array.isArray(opts.sitemaps) ? opts.sitemaps : [opts.sitemaps];
    if (sitemaps.length) {
        lines.push("");
...
export function robotsTxtHandler(opts) {
    return () => new Response(buildRobotsTxt(opts), {
