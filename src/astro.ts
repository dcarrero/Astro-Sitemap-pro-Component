      ...
      "astro:build:done": async ({ dir, pages, logger }: BuildDoneParams) => {
        ...
        if (/<sitemapindex[\s>]/.test(indexXml)) {
          for (const loc of extractLocs(indexXml)) {
            ...
            for (const u of extractLocs(subXml)) listed.add(norm(new URL(u).pathname));
          }
        } else {
          for (const u of extractLocs(indexXml)) listed.add(norm(new URL(u).pathname));
        }
        ...
        fail(
          `[sitemap-coverage] mismatch (${missing.length} missing, ${stale.length} stale):\n${lines.join("\n")}`,
        );
      },
    },
  };
